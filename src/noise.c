...
void noise_generate(chunk_t* chunk, const int x, const int z) {
	for (int a = 0; a < CHUNK_X; a++)
		for (int b = 0; b < CHUNK_Z; b++) {
			...
			if (low && grass) {
				const float plant =
					stb_perlin_fbm_noise3(s * 0.2f, 0.0f, t * 0.2f, 2.0f, 0.5f, 3) * 0.5f + 0.5f;
				if (plant > 0.8f && a > 2 && a < CHUNK_X - 2 && b > 2 && b < CHUNK_Z - 2) {
					...
					for (int dx = -1; dx <= 1; dx++)
						for (int dz = -1; dz <= 1; dz++)
							for (int dy = 0; dy < 2; dy++) {
								if (dx || dz || dy) {
									chunk_set_block(chunk, a + dx, y + log + dy, b + dz,
													BLOCK_LEAVES);
								}
							}
				} else if (plant > 0.55f) {
					...
					const block_t flowers[] = {BLOCK_BLUEBELL, BLOCK_DANDELION, BLOCK_LAVENDER,
											   BLOCK_ROSE};
			...
			const float cloud =
				stb_perlin_turbulence_noise3(s * 0.015f, 0.0f, t * 0.015f, 2.0f, 0.5f, 6);
			int scale = -1;
			if (cloud > 0.9f) {
				scale = 2;
			} else if (cloud > 0.7f) {
